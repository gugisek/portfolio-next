...
export default function contact({}: Props) {
  ...
  return (
    <section id="contact" className='text-neutral-900 font-[Lexend]'>
        ...
        <div className='bg-[#e0e0e0] min-h-[80vh]'>
            <h1 className='font-[Lexend-bold] sm:text-7xl text-6xl ml-[10%] text-white'>CONTACT</h1>
            <div className='w-full min-h-[80vh] flex md:flex-row flex-col items-center justify-center gap-9'>
              <motion.img 
                ...
                {socials.map((social, index) => (
                  <div className='flex flex-row items-center justify-center'>
                    <img src={social.name + ".png"} alt={social.name + "icon"} className='w-[25px] mr-3'/>
                      <a key={index} href={social.link} target="_blank" className="uppercase">{social.name}</a>
                  </div>
                ))}
                </div>
